...
if input.length != 2
    ...
    puts "USAGE: $bioruby Assignment4.rb <filename1> <filename2>"
    puts "---------------------------------------------\n";puts
    exit 1
end

# Function to guess the file type
def file_type(file)
  seq = []
  puts "Opening file"
  File.open(file, 'r').each do |line|
    ...
    if line[0] == '>' 
      ...
      # Transforms the sequence in a Bio::Sequence object
      seq = Bio::Sequence.auto(line)
      break
    end 
  end
  type = seq.guess.to_s.split('::')[-1] # Returns 'AA' or 'NA'
  if type == "AA"
    type_file = type.gsub("AA", "prot")
  elsif type == "NA"
    type_file = type.gsub("NA", "nucl")
  end
  return(type_file)  
end
...
# File type: nucl or prot
type_file1 = file_type(file1)
type_file2 = file_type(file2)
...
fasta_file1.each do |seq_f1| # We iterate over each sequence in the file1 
  ...
  if (report_f2.hits[0])
    if factory_f2.program == "blastx" 
      cov_f2 = ((report_f2.hits[0].query_end.to_f - report_f2.hits[0].query_start.to_f)*3)/report_f2.hits[0].query_len.to_f
    else
      cov_f2 = (report_f2.hits[0].query_end.to_f - report_f2.hits[0].query_start.to_f)/report_f2.hits[0].query_len.to_f
    end
    if (report_f2.hits[0].evalue <= $evalue) and (cov_f2 >= $coverage) 
      ...
      # Only keep the BLAST result if it passes the evalue filter
      if (report_f1.hits[0])
        if factory_f1.program == "blastx"
          cov_f1 = ((report_f1.hits[0].query_end.to_f - report_f1.hits[0].query_start.to_f)*3)/report_f1.hits[0].query_len.to_f
        else
          cov_f1 = (report_f1.hits[0].query_end.to_f - report_f1.hits[0].query_start.to_f)/report_f1.hits[0].query_len.to_f
        end
        if (report_f1.hits[0].evalue <= $evalue) and (cov_f1 >= $coverage) 
